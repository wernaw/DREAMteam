...
@app.get("/api/teams")
async def get_teams(limit: int = 1):

    if not os.path.exists("last_teams.json"):
        return []

    with open("last_teams.json", "r", encoding="utf-8") as f:
        teams = json.load(f)
...
async def generator(
    ...
    with open("last_teams.json", "w", encoding="utf-8") as f:
...
async def save_teams(
    ...
    with open("last_teams.json", "w", encoding="utf-8") as f:
        json.dump(payload.teams, f, ensure_ascii=False, indent=2)
